...
class SlamSystem(object):
    ...
    def __init__(self,
        ...
        self.extraction_thread = ExtractionThread(args=self.args.slam_system, posegraph_map=self.posegraph_map, dpm_encoder=self.dpm_encoder, label_decoder=self.label_decoder, system_info=self.system_info, device=self.device)
    ...
    def step(self, sensor_data: List[torch.Tensor]) -> EXIT_CODE:
        ...
        self.frame_id += 1      # 增加帧计数

        perf_t = time.perf_counter()
        time_ms = len(self.posegraph_map.get_all_scans()) / 10
        point_cloud, R, T, padding_mask, original_scan = detach_to_device(sensor_data, device=self.device,
                                                                          non_blocking=True)
        ...
        # 调用线程的 `process` 函数提取描述符（包括点云坐标和特征描述符）
        descriptors = self.extraction_thread.process(point_cloud=point_cloud, padding_mask=padding_mask)

        # 创建一个新的 `ScanPack` 实例，包含时间戳、代理ID、帧ID、关键点描述符、完整点云、坐标系统和姿态的真值 (SE3)
        new_scan = ScanPack(
            timestamp=time_ms,
            agent_id=self.system_id,
            timestep=self.frame_id,
            key_points=descriptors[0],  # 提取的特征和坐标描述符 [fea+xyz]
            full_pcd=point_cloud[0].clone() * self.args.slam_system.coor_scale,  # 点云坐标乘以比例尺，表示真实坐标
            coor_sys=self.coor_sys,
            SE3_gt=PoseTool.SE3(R[0], T[0]))  # 使用 R 和 T 生成真值位姿（旋转和平移矩阵）

        self.result_logger.record_perf('extract', time.perf_counter() - perf_t)

        perf_t = time.perf_counter()

        # 调用线程的 `process` 函数进行里程计处理，生成里程计边缘
        odom_edges = self.odometry_thread.process(new_scan=new_scan)

        # 如果没有生成里程计边缘，默认设置 SE3 预测为单位矩阵，表示没有位姿变化，并将当前扫描添加到位姿图中
        if len(odom_edges) == 0:
            new_scan.SE3_pred = torch.eye(4)  # 预测的位姿为单位矩阵
            self.posegraph_map.add_vertex(new_scan)  # 将当前扫描作为顶点添加到位姿图中
            self.posegraph_map.last_known_anyframe = new_scan.token  # 更新最后已知的任何帧
            self.posegraph_map.last_known_keyframe = new_scan.token  # 更新最后已知的关键帧
            odom_edge = None  # 没有里程计边缘
        else:
            # 如果有里程计边缘，记录处理时间，并选取第一个里程计边缘
            odom_edge = odom_edges[0]  # 假设里程计边缘只包含一个边缘
            self.result_logger.record_perf('odometer', time.perf_counter() - perf_t)

            # 映射处理（丢弃、关键帧处理、扫描与地图匹配）
            # 尝试：动态帧合并
            perf_t = time.perf_counter()

            # 调用 `mapping_thread` 处理函数进行映射，输入是新扫描和里程计边缘
            result = self.mapping_thread.process(new_scan=new_scan, odom_edge=odom_edge)

            # 如果结果是 `EXIT_CODE`，则直接返回该退出码
            if isinstance(result, EXIT_CODE):
                return result
            else:
                # 否则，结果应该是一个 `PoseGraph_Edge` 对象
            ...
            # 调用 `loop_thread` 处理函数进行闭环检测和优化，目标是“自身”
        ...
        # 如果通信模块存在，执行邻居边缘检测并上传扫描数据
        if (self.comm_module is not None):
            neibor_edges = []  # 存储不包含里程计边缘的邻居边缘
            ...
            for j in self.posegraph_map.get_neighbor_tokens(new_scan.token):
                ...
                # 检查邻居边缘是否已经存在于图中，如果存在则添加到列表中
                if (self.posegraph_map.has_edge(j, new_scan.token, _DEBUG_warning=False)):
                    neibor_edges.append(self.posegraph_map.get_edge(j, new_scan.token))
                elif (self.posegraph_map.has_edge(new_scan.token, j, _DEBUG_warning=False)):
            ...
            # 发送消息，包含新扫描、里程计边缘和邻居边缘信息
            self.comm_module.send_message(caller=self.comm_id, callee=0, command='UPLOAD_SCAN',
                                          message=dict(new_scan=new_scan, odometer_edge=odom_edge,
                                                       neighbor_edges=neibor_edges))
...
class CloudSystem(SlamSystem):
    ...
    def step(self, scan_pack: ScanPack, odom_edge: Optional[PoseGraph_Edge], neighbor_edges: List[PoseGraph_Edge]):
        ...
        for e in validated_loop_edges:
            if (self.posegraph_map.has_edge(e.src_scan_token, e.dst_scan_token, _DEBUG_warning=False) == False):
                self.posegraph_map.add_edge(e)
