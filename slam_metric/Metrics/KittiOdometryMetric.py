...
class KittiOdometryMetric():
    ...
    def __init__(self, dataset: str):
        ...
        if (dataset == 'kitti'):
            ...
            self.lengths = [5, 10, 15, 20, 25, 30, 35, 40]
        elif (dataset == 'carla'):
            self.lengths = [100, 200, 300, 400, 500, 600, 700, 800]
        else:
            self.lengths = [100, 200, 300, 400, 500, 600, 700, 800]
            logger.warning(f'Dataset "{dataset}" not specific, use default length: {self.lengths}')
            # raise ValueError
    ...
    def plot_error(self, avg_segment_errs, fig_name):
        ...
        plt.ylabel('Rotation Error (deg/100m)')
        plt.xlabel('Path Length (m)')
        plt.legend(loc="upper right")
        fig.set_size_inches(5, 5)
    ...
    def eval(self, gt_file, pred_file, alignment=None, fig_path=None, fig_name=None):
        ...
        # Read pose txt
        poses_result = self.load_poses_from_txt(self.pred_file)
        poses_gt = self.load_poses_from_txt(self.gt_file)
        ...
        # self.save_sequence_errors(seq_err, os.path.join(fig_path, f'{fig_name}_err.txt'))
        ...
        if (fig_path is not None and fig_name is not None):
            self.plot_error(avg_segment_errs, os.path.join(fig_path, f'{fig_name}_kitti.png'))

        return (ave_t_err * 100, ave_r_err / np.pi * 180 * 100, ate, rpe_trans, rpe_rot * 180 / np.pi)
        # Plotting
