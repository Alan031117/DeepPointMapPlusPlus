...
def main():
    ...
    gt_pose_root_kitti360 = r'/data/fengbh/data/fengbh/DPM-AAAI/dataset/GroundTruthTraj/KITTI360_poses_Lidar'
    seq_name_kitti = ['06', '07', '08', '09', '10']
    seq_name_kitti360 = ['07', '08', '09', '10', '18']

    seq_list = sorted(list(glob(os.path.join(pred_pose_root, '*'))))
    if args.kitti360_only:
        seq_list_kitti = []
        seq_list_kitti360 = seq_list
    else:
        seq_list_kitti = seq_list[:5]
        seq_list_kitti360 = seq_list[5:]
        if len(seq_list_kitti) < 5:
            seq_name_kitti = seq_name_kitti[5-len(seq_list_kitti):]

    # KITTI
    for i, seq in enumerate(seq_list_kitti):
    ...
    # KITTI360
    for i, seq in enumerate(seq_list_kitti360):
        gt_file_path = os.path.join(gt_pose_root_kitti360, f'pose_lidar_2013_05_28_drive_00{seq_name_kitti360[i]}_sync.gt.txt')
        pred_file_path = os.path.join(seq, r'trajectory.allframes.txt')
        step_file_path = os.path.join(seq, r'trajectory.allsteps.txt')
        results = eval_ape(gt_file_path, pred_file_path, step_file_path, dataset_type='kitti360')
        print(f'KITTI360 {seq_name_kitti360[i]}: {results}')
